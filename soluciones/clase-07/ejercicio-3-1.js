...
// Recorrida del array:
for (var i = 0; i < aImagenes.length; i++) {
	html += '<div>';
		html += '<a href="items/' + aImagenes[i] + '" target="_blank">';
			html += '<img src="items/' + aImagenes[i] + '" alt="' + aNombres[i] + '" />';
		html += '</a>';
		html += '<h3>' + aNombres[i] + '</h3>';
		html += '<p>' + aLanzamientos[i] + '</p>';
	html += '</div>';
}